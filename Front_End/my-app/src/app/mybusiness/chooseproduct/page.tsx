...
function ListProduct() {
    ...
    useEffect(() => {
        const fetchProduct= async ()=>{
            const res= await PartnerAPI.getRemainProducts(cusID)
    ...
    const handleNextPage = () => {
      ...
      const handleAddBtn=async (prod: string, index: number, price: number)=>{
        const a1 = products.slice(0, index);
        const a2 = products.slice(index + 1, products.length);
        const new_arr = a1.concat(a2);
        setProducts(new_arr)
        const res = await PartnerAPI.addProduct(cusID, prod, {price:price.toString()})
                ...
                <div className={cx('list_product-content')}>
                    ...
                    {currentProducts.map((product:any, index) => {
                        ...
                        <div key={product.ID_PRODUCTS} className={cx('list_product-product')} >
                            ...
                            <button className={cx("product-btn__update")} onClick={()=>handleAddBtn(product.ID_PRODUCTS,index, product.PRICE)} >
